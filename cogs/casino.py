    ...
    async def slots(self, interaction: discord.Interaction, amount: int = 1):
        ...
        if interaction.guild:
            ...
            if user_cookies < amount:
                return await interaction.response.send_message(f"You don't have enough cookies to make this wager!")
            else:
                self.db.set_value(interaction.user.id, interaction.guild.id, "cookies", user_cookies - amount)
        ...
        def evaluate_slots(wheel1, wheel2, wheel3):
            if wheel1 == wheel2 == wheel3:
                if interaction.guild:
                    payout = amount * 10
                    self.db.set_value(interaction.user.id, interaction.guild.id, "cookies", payout * 10)
                    return f"You won! Payout: {payout}"
            else:
                return "Tough luck!"

        await interaction.response.send_message(f"{wheel1} | {wheel2} | {wheel3}... {evaluate_slots(wheel1, wheel2, wheel3)}")
